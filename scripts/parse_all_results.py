...
from config import METADATA_JSON, BASE_URL, DEFAULT_TARGET_YEAR, build_query_url


def parse_all_results(target_year=DEFAULT_TARGET_YEAR):
    """
    Scrapes EUR-Lex for a given year, saves results to eurlex_results.json,
    merges new entries with existing ones, and sorts by descending date.
    """
    ...
    # 1) Load first page, find total pages
    driver.get(build_query_url(target_year, 1))
    time.sleep(5)
    ...
    try:
        ...
        print(f"[i] Total results: {total_results}, Total pages: {total_pages}")
    ...
    for new in results_data:
        celex = new["celex"]
        if celex not in existing_dict:
            existing_dict[celex] = new
            added_or_updated += 1
        else:
            old = existing_dict[celex]
            if any(old.get(k) != new.get(k) for k in ["title", "link", "date", "page"]):
                existing_dict[celex] = new
                added_or_updated += 1
